...
There are a couple independent variables that would relate to eachother. Income and pct unemployed relate. Bachelors degrees and pct unemployed relate. Bachelors and income relate. Pct white does not correlate with any of them. Age could correlate with income, but since the medians are around the working age I would say it doesn't corrrelate. It would make a difference if the median age was significantly higher or lower, around retirement age or school age. The variables that seem to have a linear relationship with the dependent variable are pct unemployed, income, pct bachelors, age, and pct white.
...
Overall this model is useful. The variables that are statistically significant are income, age, and pct white. The variables that are not significant are pct bachelors, population, and unemployment. If all you are looking at is the linear relationship then you are unable to tell which of the variables are actually statistically significant in affecting voter turnouter. I believed based off the linear relationship that unemployment would be statistically significant, but it is not although there is a negative relationship. It is important to compare the variables up against eachother in how they impact voter turnout.
...
The first model I only put two of the variables that were statistically significant and you can see how that turnout is right below the full model. The second one I did only pct white because that was one of the statistically significant variables. I then did income and pct bachelors because initially I thought those two would be ones that impacted voter turnout the most, but you can see that it is the lowest of the three models. 

```{r}
# Model 2
model2 <- lm(turnout ~ median_age + income, data = md_turnout)
summary(model2)
...
When covering an election it is important to use predictors so that you can have a story ready to be published as soon as the election closes and winners are officially announced. You can compare the voter turnout to how people with variables typically vote to predict. It is all just prediction though.

It would better be used for a voter participation story and predicting what types of people will be voting in upcoming elections.
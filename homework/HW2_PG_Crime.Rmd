...
All categories involving incidents with cars and more significant than crimes not dealing with cars. Auto theft and theft from cars are the two most significant. These are important to know so that people know to protect their cars and always lock them whenever leaving their vehicle. I don't think any of them make summarizing the data more difficult because each category is relatively similar across the months. 
...
The category with the highest average is accidents. The lowest is residential robbery. This tells us that incidents involving cars is a more common crime in PG County since the top three are accidents, auto stolen, and theft from auto. I have no questions about any categories. 
...
summer_crime_averages <- pg_crime |> 
  ...
  mutate(july_change = (Jul- Jun)/Jun * 100) |>
          mutate(august_change = (Aug- Jun)/Jun * 100)
```
...
Violent crimes are most likely to occur in the summer months with assault with weapons and shootings having the highest positive percentage change. The general theory that crime is more likely to occur during the summer is not supported by the data though as a lot of the crimes have a negative percentage change. That theory might come from the fact that violent crime is more likely and that is often what receives more coverage. 
...
There are on average 481 car accidents in PG County per month.  

Drivers should be alert on roads at all times as this is the leading crime in PG county. Cars being stolen is the second leading crime that drivers should be aware of. 

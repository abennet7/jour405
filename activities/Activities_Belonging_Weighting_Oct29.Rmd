...
There are various statements that raise concerns about selection bias and reporting bias. The way different complex identities were lumped together also may create problems.  
...
```{r load-data}
# Load the dataset
nonvoters_data <- read_csv("https://raw.githubusercontent.com/dwillis/jour405_files/main/nonvoters_data.csv")
...
1. What is the range of weights in this dataset? **3.0386 - 0.2298 = 2.8088:**

2. What does it mean when a respondent has a weight of 0.23? What about 3.04? **For a respondant with a weight of .23 it means that it is overrepresented. For a weight of 3.04 it means it is underrepresented.** 

3. Why might some respondents be weighted more heavily than others? **Some respondants are weighted more heavily to correct unequal probabilities of selection.**
...
1. Which voter category shows the biggest difference between weighted and unweighted results? **Rarely/never shows the biggest % point difference.**

2. How might these differences affect reporting on likely voter turnout? **Reporting on likely voter turnout might not be as accurate due to not having an accurate depictions of who would go and vote from the "rarely/never" category.**

3. Why do you think these differences exist? What does this tell us about who might be over or under-represented in the sample? **People in the rarely/never category might be underrepresented. I think these differences exist because the weights can't be entirely accurate.**

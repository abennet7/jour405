...
**Yes it is a useful model. The f-statistic says the model as a whole is statistically significent so then it would be useful. 62.32% of the variability in wins, which is the majority, is explained by the model which means it could be useful. 58.28% of the variance is also explained which is the majority.**
...
*Out of the three factors, yards gained, takeaways, and points scored the one that has the most impact on wins is points scored. The one that was least impactful is yards gained. There is not one that is not impactful at all but there is a clear ranking in which is more impactful than the others.*

## Using the Model

Without using the words "multiple regression model", describe how you could use this model to cover a team or game.

This model showcases how much or how little certain factors impact others. The dark blue circle means it correlates the most. You will see wins and wins correlate the most because they are the same thing. No circle means there is no impact. Takeaways and yards gained do not impact eachother at all so there is no circle. You can use it to cover a game and use it as a prediction model. You can explain why a team is struggling or suceeding with it.